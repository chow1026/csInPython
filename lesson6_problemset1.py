...
def rabbits(n):
    ...
    i = 1
    while i < n:
        keep_five.append(keep_five[-1] + keep_five[-2])
        if len(keep_five) > 5:
            keep_five[-1] = keep_five[-1] - keep_five[0]
            keep_five.pop(0)
        i += 1
    return keep_five[-1]
...
def hexes_to_udaciousness(n, spread, target):
    if n > target:
        ...
        if n + n * spread < target:
            # print(n)
            return 1 + hexes_to_udaciousness((1 + spread) * n, spread, target)
        else:
            return 1
